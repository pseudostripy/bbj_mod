 ...
 *            exports.DEF nobbj.c -shared -o DINPUT8.dll
 *
 */
...
{ 	
	...
	*/
	...
	//	
	memcpy(&spd_bytes[13],&speedBse,4); 			// fix mov [speedBse],esi
	...
	*/
	...
	void *bbjinj_red = bbjinjaddr + 8;
	
	// Calc and update addresses
	jmpfw_offset 	= jumphook - (bbjinjaddr + 5);
	jmpret_offset 	= bbjinj_red - ((void*)&jump_bytes[23] + 5);
	
	memcpy(&jump_patch.patch[1],&jmpfw_offset,4); 		// jump to spd_bytes code
	memcpy(&jump_bytes[24],&jmpret_offset,4); 			// return to spdinject (+offset)
	//	
